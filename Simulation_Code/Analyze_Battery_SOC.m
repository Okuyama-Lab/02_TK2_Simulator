...
function Analyze_Battery_SOC(DT)
    ...
    %% === グラフ①：バッテリーSOC ===
    figure;
    plot(time, Capacity, 'Color', [0 0 1], 'LineWidth', 1.5);
    ...
    grid on;
    legend('SOC [mAh]', 'Max Capacity');
    saveas(gcf, fullfile(outdir, 'Battery_SOC.png'));
    saveas(gcf,fullfile(outdir,'Battery_SOC.fig'));

    %% === グラフ②：消費電力履歴 ===
    figure;
    plot(time, Total_power_log, 'Color', [1 0 0], 'LineWidth', 1.2);
    xlabel('Time [s]');
    ylabel('Total Power [W]');
    title('Total Power Consumption History');
    grid on;
    ...
    %% === グラフ③：発電量履歴 ===
    figure;
    plot(time, Power, 'Color', [1 0 0], 'LineWidth', 1.2);
    xlabel('Time [s]');
    ylabel('Generated Power [W]');
    title('Power Generation History');
    grid on;
